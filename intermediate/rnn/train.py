"""
Training script for the vanilla RNN on 20 Newsgroups (4-category subset).
Run from repo root: python -m intermediate.rnn.train
"""
...
from intermediate.rnn.model import RNNModel
...
def load_data():
    """Loads a 4-category subset of 20 Newsgroups, splits train/val/test."""
    ...
    # Filter out empty texts after removing headers/footers/quotes
...
def train_one_epoch(model, loader, optimizer, criterion):
    ...
    for inputs, labels in loader:
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)  # RNN stability
        optimizer.step()
...
def main():
    ...
    model = RNNModel(vocab_size=len(vocab), num_classes=len(class_names)).to(DEVICE)
    ...
    print(f"Training RNN on {len(train_ds)} samples, validating on {len(val_ds)}...")
    ...
    torch.save(model.state_dict(), RESULTS_DIR / "rnn_model.pt")

    plot_training_curves(
        train_losses, val_losses, train_accs, val_accs,
        title="RNN — 20 Newsgroups Classification",
        save_path=RESULTS_DIR / "training_curves.png"
    )

    results = evaluate_model(model, test_loader, DEVICE, class_names=class_names)
    print_evaluation_summary(results, model_name="RNN")
